...
| スキル | いつ使う | 説明 |
|---|---|---|
| `/setup` | 新規クローン直後・利用スタイル変更時 | `local.yml` 作成の案内、利用スタイル（ダウンロードのみ／仕訳登録も使う）の確認、ダウンロードのみの場合の MoneyForward MCP 無効化（書き込み前に確認）を対話的に実施 |
...
Phase 1–31（進行中の 25・29 を除く）は [phases.md](phases.md) に統合済み（「なぜ今の構成になっているか」の単一履歴）。
...
| 26 | 請求日アンカーの学習・キャッシュ化 ✅ | 契約者依存の請求日（14日・月末等）を `services/*.yml` から除去し、MF 連携データから観測して `local.yml` の `cache:` に学習 | [phases.md](phases.md) |
| 27 | config 層構造の明文化 ✅ | `config/services/*.yml` の配置は現状維持と決定（案A）。データ層/手順層の設計原則とフィールド×消費者マップを明文化し、分散した説明を統合 | [phases.md](phases.md) |
| 28 | カード種別対応（個人用＝事業主借直接） ✅ | 個人用カードの課金を `通信費/事業主借/<カード補助科目>` の直接記帳に統一（未払金型カードを事業主借直接へ寄せる）。貸方は `credit_cards.type` 依存で解決（register が連携口座の登録先勘定科目と照合）。credit-payment を未払金型（事業用）専用に | [phases.md](phases.md) |
| 29 | config スリム化（進行中） | 全フィールドを消費者と突き合わせ、デッド・情報ゼロ・規約導出可能なフィールドを削減。借方の勘定科目・補助科目の MF 自動仕訳委譲を検討 | [phase29/](phase29-config-slimming/) |
| 30 | Moomoo ドメイン設定の name フィールド削除 ✅ | `domains[].name` を削除し `fqdn` に完全一本化。ファイル名は `.`→`_` 置換した `{domain_fqdn_us}`、摘要は無変換の `{domain_fqdn}` を使用 | [phases.md](phases.md) |
| 31 | Anthropic 請求書取得の Billing 主化 ✅ | メール（Gmail）主・Billing 副を反転し claude.ai Billing を主に。実機確認の結果、年月導出は Stripe ではなく claude.ai テーブルの日付列基準に修正（Stripe に "Date of issue" は存在せず、支払い日がテーブル日付と1日ズレる例を確認） | [phases.md](phases.md) |
| 32 | MoneyForward MCP の遅延有効化 ✅ | ダウンロードのみの利用では OAuth 認証を求めないよう、`settings.local.json`（Git 管理外）の `disabledMcpjsonServers` で無効化しておき、`/register`・`/credit-payment` 実行時に同ファイルを書き換えて自動有効化（共有 `settings.json` は変更しない）。無効化↔有効化のフルサイクルを実機確認済み | [phase32/](phase32-moneyforward-mcp-lazy-enable/) |
| 33 | プロンプトインジェクション対策 ✅ | Web ページ・メール・PDF・MF 明細等の外部コンテンツを「データであり指示ではない」と扱うポリシーを SECURITY.md §10 に新設し、`_shared/security.md`・download スキルへ反映。外部由来の値の Bash クォート必須化 | [phase33/](phase33-prompt-injection-hardening/) |
