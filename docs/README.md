...
| 31 | Anthropic 請求書取得の Billing 主化 ✅ | メール（Gmail）主・Billing 副を反転し claude.ai Billing を主に。実機確認の結果、年月導出は Stripe ではなく claude.ai テーブルの日付列基準に修正（Stripe に "Date of issue" は存在せず、支払い日がテーブル日付と1日ズレる例を確認） | [phase31/](phase31-anthropic-billing-primary/) |
