...
| 31 | Anthropic 請求書取得の Billing 主化（進行中） | メール（Gmail）主・Billing 副を反転し claude.ai Billing を主に。Gmail の巨大スナップショットを避けてトークン消費を削減。PDF は両ルートとも Stripe 生成の同一 Invoice | [phase31/](phase31-anthropic-billing-primary/) |
